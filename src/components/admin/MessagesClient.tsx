...
export default function MessagesClient({ initialMessages }: Props) {
  ...
  function handleStatusChange(id: string, status: ContactMessageStatus) {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        flash(id, 'ok', 'Status updated.')
      } else {
        flash(id, 'err', result.error ?? 'Failed to update.')
      }
    })
  }
  ...
  function handleDelete(id: string) {
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        flash(id, 'ok', 'Message deleted.')
      } else {
        flash(id, 'err', result.error ?? 'Failed to delete.')
      }
    })
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <p className="text-xs text-muted">{filtered.length} message{filtered.length !== 1 ? 's' : ''}</p>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" role="dialog" aria-modal="true" aria-label="Confirm deletion">
          ...
          <div className="relative z-10 w-full max-w-sm card-cosmic p-6 flex flex-col gap-4">
            <h2 className="font-serif text-cream font-semibold">Delete Message?</h2>
            <p className="text-silver text-sm">This will permanently remove the message. This cannot be undone.</p>
